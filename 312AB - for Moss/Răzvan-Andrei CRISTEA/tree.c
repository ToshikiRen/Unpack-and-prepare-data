...
/**
 * CERINTA 2
 * TODO: implementarea ITERATIVA functiei de parcurgere SRD(in ordine) a unui arbore binar.
...
void inorderIterative(TreeNode *root, TreeNode **arr, int size)
{
    unsigned int r = 0;
    Stack *stiva_rezerva;
    stiva_rezerva = createStack(size);
    while (1 != 0)
    {
        while (root != NULL)
        {
            push(stiva_rezerva, root);
            root = root->left;
        }
        if (isEmpty(stiva_rezerva))
            break;
        root = pop(stiva_rezerva);
        arr[r] = root;
        r++;
        root = root->right;
    }
