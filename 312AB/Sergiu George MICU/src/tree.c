...
int getSize(TreeNode* root){
    if(root == NULL)
        ...
        return 1 + max(getSize(root->left), getSize(root->right));
}

int getPath(TreeNode** path, TreeNode* root, TreeNode* target, int iterator){
    int truth;
    ...
    if(target == root)
        ...
        truth = getPath(path, root->left, target, iterator + 1) || getPath(path, root->right, target, iterator + 1);
...
TreeNode* LCA(TreeNode* root, TreeNode* n1, TreeNode* n2) {
    ...
    TreeNode** path1 = (TreeNode**) calloc(size, sizeof(TreeNode*));
    TreeNode** path2 = (TreeNode**) calloc(size, sizeof(TreeNode*));

    for(i = 0; i < size; i++)
        path1[i] = path2[i] = NULL;

    getPath(path1, root, n1, 0);
    getPath(path2, root, n2, 0);

    for(i = 0; i < size && path2[i] != NULL && path1[i] != NULL; i++)
        if(path2[i] != path1[i])
            break;
